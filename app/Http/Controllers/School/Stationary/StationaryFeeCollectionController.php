...
class StationaryFeeCollectionController extends Controller
{
    ...
    public function show(StationaryStudentAllocation $allocation)
    {
        ...
        $concessions = \App\Models\FeeConcession::where('school_id', $allocation->school_id)
            ...
            ->where(fn ($q) => $q->where('is_one_time', false)->orWhereDoesntHave('stationaryPayments'))
    ...
    public function store(Request $request, StationaryStudentAllocation $allocation)
    {
        ...
        if ($concessionId) {
            ...
            if ($concession && $concession->is_one_time && \App\Models\StationaryFeePayment::where('concession_id', $concessionId)->exists()) {
