...
class LedgerController extends Controller
{
    ...
    public function dayBook(Request $request)
    {
        ...
        // Inflows (Stationary Fee Collections)
        $stationaryQuery = StationaryFeePayment::where('school_id', $schoolId)
            ->whereDate('payment_date', '>=', $startDate)
            ->whereDate('payment_date', '<=', $endDate)
            ->where('amount_paid', '>', 0)
            ->with(['student.currentAcademicHistory.courseClass', 'student.currentAcademicHistory.section', 'allocation']);
     ...
     * school has configured as active. Balance is sourced from DueReportService
     * so the amount in the message matches what the Due Report screen shows —
     * tuition + transport + hostel + stationary combined.
     */
    public function sendDueReminder(Request $request, DueReportService $service)
    {
        ...
        // Pull combined balances (all 4 streams) for the requested students.
        // The service returns total_balance = tuition + transport + hostel + stationary.
        $rows = $service->rowsFor(
            schoolId:       $schoolId,
        ...
        foreach ($students as $student) {
            $row     = $rowsByStudent->get($student->id);
