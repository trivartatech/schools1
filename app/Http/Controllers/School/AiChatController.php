...
class AiChatController extends Controller
{
    ...
    private function getLiveDataSummary(): string
    {
        try {
            ...
            // ── FEES ──────────────────────────────────────────────────────
            $feeToday  = FeePayment::where('school_id', $sid)->where('academic_year_id', $yid)->where('payment_date', $today)->where('status', 'paid')->sum('amount_paid');
            $feeMonth  = FeePayment::where('school_id', $sid)->where('academic_year_id', $yid)->where('payment_date', '>=', $monthStart)->where('status', 'paid')->sum('amount_paid');
            $feeYear   = FeePayment::where('school_id', $sid)->where('academic_year_id', $yid)->where('status', 'paid')->sum('amount_paid');
