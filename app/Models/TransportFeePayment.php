...
/**
 * A single transport-fee receipt.
 *
 * Receipt numbers are generated from the school's configurable
 * `transport_receipt_*` settings (prefix/suffix/start_no/pad_length),
...
class TransportFeePayment extends Model
{
    ...
    public function allocation()    { return $this->belongsTo(TransportStudentAllocation::class, 'allocation_id'); }
     ...
     * so the unified notification flow works for transport + tuition + hostel.
     */
    ...
    protected static function booted(): void
    {
        static::creating(function (self $payment) {
            if (empty($payment->receipt_no)) {
                ...
                $startNo   = (int) ($settings['transport_receipt_start_no']   ?? 1);
                $padLength = (int) ($settings['transport_receipt_pad_length'] ?? 5);
     ...
     * that transport and regular receipts support identical formatting.
     */
