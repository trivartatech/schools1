#!/bin/bash
# =============================================================================
# Fresh-deploy script for National English School, Chitradurga
# Horpete, Vijayanagar, Chitradurga, Karnataka 577502
#
...
#   2. .env created from .env.nes-production.example with DB_* filled in
#   3. If DB_ROOT_PASSWORD is set, the DB will be created automatically;
#      otherwise create the database manually before running.
# =============================================================================
set -e

echo "🚀 National English School, Chitradurga — Fresh Deploy"

if [ ! -f .env ]; then
    echo "❌ .env missing. Copy .env.nes-production.example → .env and fill MySQL creds first."
    exit 1
fi
...
echo "🌱 Seeding school (National English School)…"
php artisan db:seed --class=NationalEnglishSchoolSeeder --force
...
echo ""
echo "✅ National English School deploy complete."
echo ""
echo "Login at: ${APP_URL}/login"
echo "  Super admin: superadmin@nescta.in"
echo "  School admin: admin@nescta.in"
echo "  Principal:    principal@nescta.in"
