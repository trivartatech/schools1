...
<head>
    ...
    <title>Verify Hostel Receipt</title>
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="max-w-md w-full bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        <div class="bg-blue-600 px-6 py-4 text-center">
            ...
            <h1 class="text-xl font-bold text-white">Verified Hostel Receipt</h1>
        ...
        <div class="p-6 space-y-4">
            ...
            @if($payment->allocation?->bed?->room?->hostel)
            <div class="flex justify-between border-b pb-3">
                <span class="text-gray-500 font-medium">Hostel</span>
                <span class="text-gray-900">{{ $payment->allocation->bed->room->hostel->name }}</span>
            </div>
            @endif
            @if($payment->allocation?->bed?->room?->room_number)
            <div class="flex justify-between border-b pb-3">
                <span class="text-gray-500 font-medium">Room / Bed</span>
                <span class="text-gray-900">
                    Rm {{ $payment->allocation->bed->room->room_number }}
                    @if($payment->allocation?->bed?->name) · {{ $payment->allocation->bed->name }} @endif
                </span>
            </div>
            @endif
            <div class="flex justify-between border-b pb-3">
        ...
        <div class="bg-gray-50 px-6 py-4 text-center text-xs text-gray-500 border-t">
            This digital receipt confirms the hostel fee transaction stored in the official School ERP database.
        </div>
    </div>
