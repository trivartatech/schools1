...
<head>
    ...
    <title>Verify Stationary Receipt</title>
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="max-w-md w-full bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        <div class="bg-amber-600 px-6 py-4 text-center">
            <div class="inline-block bg-white text-amber-600 rounded-full p-2 mb-2">
                <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                </svg>
            </div>
            <h1 class="text-xl font-bold text-white">Verified Stationary Receipt</h1>
            <p class="text-amber-100 text-sm">Issued by {{ $payment->school->name ?? 'School' }}</p>
        </div>
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="flex justify-between border-b pb-3">
                ...
                <span class="font-bold text-amber-600 text-lg">₹{{ number_format($payment->amount_paid, 2) }}</span>
            </div>
        ...
        <div class="bg-gray-50 px-6 py-4 text-center text-xs text-gray-500 border-t">
            This digital receipt confirms the stationary fee transaction stored in the official School ERP database.
        </div>
    </div>
