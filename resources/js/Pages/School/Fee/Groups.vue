...
// Head form
const newHead = useForm({ fee_group_id: '', name: '', short_code: '', is_taxable: false, gst_percent: 0, is_transport_fee: false, is_hostel_fee: false });
        ...
        <div class="max-w-5xl mx-auto space-y-6">
            ...
            <div v-for="group in groups" :key="group.id" class="card overflow-hidden">
                ...
                <div v-if="showNewHead === group.id" class="px-5 py-4 bg-blue-50 border-b border-blue-100 flex flex-wrap items-end gap-3">
                    ...
                    <div class="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg" :class="newHead.is_transport_fee ? 'bg-blue-100 border border-blue-300' : 'bg-white border border-gray-200'">
                        <button type="button"
                            @click="newHead.is_transport_fee = !newHead.is_transport_fee; if(newHead.is_transport_fee) newHead.is_hostel_fee = false"
                            :class="newHead.is_transport_fee ? 'bg-blue-600' : 'bg-gray-300'"
                            ...
                            <span :class="newHead.is_transport_fee ? 'translate-x-4' : 'translate-x-0'"
                                  class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"></span>
                        </button>
                        <span class="text-xs font-semibold" :class="newHead.is_transport_fee ? 'text-blue-700' : 'text-gray-500'">🚌 Transport Fee Head</span>
                        <span v-if="newHead.is_transport_fee" class="text-[10px] text-blue-500">(Only one allowed per school — auto-links transport allocations)</span>
                    </div>
                    <!-- Hostel Fee toggle -->
                    <div class="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg" :class="newHead.is_hostel_fee ? 'bg-purple-100 border border-purple-300' : 'bg-white border border-gray-200'">
                        <button type="button"
                            @click="newHead.is_hostel_fee = !newHead.is_hostel_fee; if(newHead.is_hostel_fee) newHead.is_transport_fee = false"
                ...
                <div v-if="group.fee_heads && group.fee_heads.length">
                    <div v-for="head in group.fee_heads" :key="head.id"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span v-if="head.is_transport_fee" class="badge badge-blue">🚌 Transport</span>
