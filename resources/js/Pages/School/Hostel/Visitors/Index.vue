...
<template>
    <SchoolLayout title="Hostel Visitors">
        ...
        <PageHeader title="Visitor Logs" subtitle="Manage hostel visitor entry & exit with photo verification.">
            <template #actions>
        ...
        <!-- Table -->
        <div class="card">
            <Table :empty="!visitors.data.length">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Visitor</th>
                ...
                <template #empty>
                    <EmptyState
                        ...
                        description="Click '+ Add Visitor' to log the first visitor entry."
                        action-label="+ Add Visitor"
                        @action="openModal()"
                    />
                </template>
            </Table>
        ...
        <!-- ADD / MARK OUT MODAL -->
        <Modal v-model:open="showModal" :title="editing ? 'Mark Visitor Out' : 'New Visitor Log'" size="md">
            <form @submit.prevent="save" id="visitor-form">
            ...
            <template #footer>
                ...
                <Button type="submit" form="visitor-form" :loading="loading">{{ editing ? 'Mark Out' : 'Log Visitor' }}</Button>
            </template>
        </Modal>

        <!-- VISITOR PASS + QR -->
        <Modal v-if="selectedVisitor" v-model:open="showPassCard" title="Visitor Pass" size="sm">
            <VisitorPassCard :visitor="selectedVisitor" />
        </Modal>

        <!-- WEBCAM CAPTURE (independent component, not a modal) -->
        <WebcamCapture
            v-if="showWebcam"
