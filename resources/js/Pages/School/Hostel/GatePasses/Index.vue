...
// ----- Delete -----
async function destroy(gp) {
    const ok = await confirm({
        title: 'Delete gate pass?',
        message: 'This will permanently remove the pass and any captured photos.',
        confirmLabel: 'Delete',
        danger: true,
    });
    if (!ok) return;
...
<template>
    <SchoolLayout title="Gate Passes">
        ...
        <PageHeader title="Gate Passes" subtitle="Student leave & exit management with escort tracking.">
            <template #actions>
        ...
        <!-- Table -->
        <div class="card">
            <Table :empty="!gatePasses.data.length">
                <thead>
                    <tr>
                        <th>Student</th>
                        <th>Type & Duration</th>
                ...
                <template #empty>
                    <EmptyState
                        ...
                        description="No gate-pass requests have been recorded yet."
                        action-label="+ New Gate Pass"
        ...
        <!-- ===== CREATE MODAL ===== -->
        <Modal v-model:open="showCreateModal" title="New Gate Pass Request" size="lg">
            <form @submit.prevent="saveGatePass" id="create-gp-form">
                <div class="form-row">
            ...
            <template #footer>
                ...
                <Button type="submit" form="create-gp-form" :loading="createLoading">Create Gate Pass</Button>
            </template>
        </Modal>

        <!-- ===== STATUS MODAL (Out / Return) ===== -->
        <Modal v-model:open="showModal" :title="actionData.status === 'Out' ? 'Mark Student Out' : 'Mark Student Returned'" size="sm">
            <form @submit.prevent="saveStatusUpdate" id="status-form">
            ...
            <template #footer>
                ...
                <Button type="submit" form="status-form">Save</Button>
            </template>
        </Modal>

        <!-- ===== OTP MODAL ===== -->
        <Modal v-model:open="showOtpModal" title="Parent OTP Verification" size="sm">
            <p style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 1.25rem; text-align: center;">
                Enter the 6-digit OTP sent to parent's mobile
            </p>
        ...
        <Modal
            ...
            :title="`Gate Pass #${selectedPass.id} — Details`"
            size="md"
        >
            <!-- Status badges -->
        ...
        <!-- ===== GATE PASS CARD + QR ===== -->
        <Modal v-if="selectedPass" v-model:open="showPassCard" title="Digital Gate Pass" size="sm">
