...
async function batchPostGl() {
    const ok = await confirm({
        ...
        message: 'Post all unsynced hostel-fee receipts to the General Ledger?',
        confirmLabel: 'Post All',
    });
    if (!ok) return;
    batchPostingGl.value = true;
    router.post('/school/hostel/fees/batch-post-gl', {}, {
        preserveScroll: true,
...
<template>
    ...
    <SchoolLayout title="Hostel Fee Collection">
        ...
        <!-- Header -->
        <PageHeader title="🏠 Hostel Fee Collection" subtitle="Every hostel allocation has its own outstanding balance. Receipts are numbered separately from regular fees.">
            <template #actions>
                <Button variant="secondary" @click="batchPostGl" :loading="batchPostingGl" title="Post all unsynced hostel-fee receipts to General Ledger">
