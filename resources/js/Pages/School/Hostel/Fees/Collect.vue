...
// When a concession is picked, auto-fill discount from its rule
...
function submit() {
    form.post(`/school/hostel/fees/${props.allocation.id}/collect`, {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <Head :title="`Hostel Fee — ${studentName}`" />
    <SchoolLayout title="Hostel Fee Collection">

        <!-- Breadcrumb -->
        <div class="mb-3 text-sm">
            <Link href="/school/hostel/fees" class="text-indigo-600 hover:underline">← Back to Hostel Fees</Link>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 mb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                <p class="text-[11px] uppercase tracking-wider text-gray-500">Hostel</p>
                <p class="font-semibold text-gray-800 mt-1">{{ allocation.bed?.room?.hostel?.name ?? '—' }}</p>
            </div>
            <div>
                <p class="text-[11px] uppercase tracking-wider text-gray-500">Room / Bed</p>
                <p class="font-semibold text-gray-800 mt-1">
                    <span v-if="allocation.bed?.room?.room_number">Rm {{ allocation.bed.room.room_number }}</span>
                    <span v-if="allocation.bed?.name"> · {{ allocation.bed.name }}</span>
                </p>
                <p v-if="allocation.bed?.room?.cost_per_month" class="text-xs text-gray-500">
                    {{ fmt(allocation.bed.room.cost_per_month) }} / month
                </p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
                <p class="text-[11px] uppercase tracking-wider text-indigo-600">Hostel Fee</p>
                <p class="text-lg font-bold text-indigo-700 mt-1">{{ fmt(allocation.hostel_fee) }}</p>
            </div>
        ...
        <!-- Collection form -->
        <div class="bg-white rounded-xl border border-gray-200 p-5 mb-5" v-if="can('collect_hostel_fee') && Number(allocation.balance) > 0">
        ...
        <div v-else-if="Number(allocation.balance) <= 0"
            ...
            ✅ No outstanding balance — all hostel fees for this allocation are settled.
        </div>
