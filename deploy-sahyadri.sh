#!/bin/bash
# =============================================================================
# Fresh-deploy script for Sahyadri School
# erp.sahyadrischool.net.in   (Site user: sahyadrischool-erp)
#
...
#   2. .env created from .env.sahyadri-production.example with DB_* filled in
#   3. If DB_ROOT_PASSWORD is set, the DB will be created automatically;
#      otherwise create the database manually before running.
# =============================================================================
set -e

echo "🚀 Sahyadri School — Fresh Deploy"

if [ ! -f .env ]; then
    echo "❌ .env missing. Copy .env.sahyadri-production.example → .env and fill MySQL creds first."
    exit 1
fi
...
echo "🌱 Seeding school (Sahyadri)…"
php artisan db:seed --class=SahyadriSchoolSeeder --force
...
echo ""
echo "✅ Sahyadri School deploy complete."
echo ""
echo "Login at: ${APP_URL}/login"
echo "  Super admin: superadmin@sahyadrischool.net.in"
echo "  School admin: admin@sahyadrischool.net.in"
echo "  Principal:    principal@sahyadrischool.net.in"
