...
return [
    ...
    */
    ...
    // Auth is handled via our custom token callback in AppServiceProvider.
    // Setting this false lets the callback decide access in all environments.
    'require_auth_in_production' => false,

    /*
    |--------------------------------------------------------------------------
    | Log Viewer Secret Token
    |--------------------------------------------------------------------------
    | Access the log viewer at /log-viewer?token=<your_secret>.
    | Once verified the token is stored in the session so you won't need to
    | pass it again until the session expires.
    | Generate a strong random string and set LOG_VIEWER_SECRET in your .env.
    |
    */
    ...
    */
    ...
    'back_to_system_label' => 'School ERP',
